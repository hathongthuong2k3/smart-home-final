  ...
  try {
    ...
    const [devices] = await db.promise().query(
      `SELECT d.ID as DeviceID, d.DName, d.DType, d.RoomID, d.HomeID, d.APIKey,
              m.MType AS CurrentMode
       FROM Device d
       LEFT JOIN (
           SELECT md.DeviceID, mo.MType
           FROM ModeDevice md
           JOIN Mode mo ON md.ModeID = mo.ID
           WHERE md.ModeID = (
               SELECT MAX(md2.ModeID)
               FROM ModeDevice md2
               WHERE md2.DeviceID = md.DeviceID
           )
       ) m ON d.ID = m.DeviceID
